...
class PipelineService:
    ...
    async def run(self, shipment: ShipmentRequest, request_id: str) -> PipelineResponse:
        ...
        usitc, comtrade = await asyncio.gather(self.usitc_lookup(classification.hs_code, requirements.corridor, request_id), self.comtrade_lookup(classification.hs_code, requirements.corridor, request_id))
        tariff = usitc if usitc.duty_rate is not None else comtrade
        documents = await self.ai.generate_documents(shipment, classification, tariff, requirements.required_docs, request_id)
